    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `Question` (`id` INTEGER PRIMARY KEY AUTOINCREMENT, `label` TEXT, `content` TEXT NOT NULL)');
...
class _$QuestionDao extends QuestionDao {
  ...
  @override
  Future<Question?> deleteQuestion(int id) async {
    return _queryAdapter.query('DELETE FROM Question WHERE id = ?1',
        mapper: (Map<String, Object?> row) => Question(
            id: row['id'] as int?,
            label: row['label'] as String?,
            content: row['content'] as String),
